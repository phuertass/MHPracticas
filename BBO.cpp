        ...
        for(int i=0; i<TAMANIO_POBLACION; i++)
        {
            ...
            {   
                ...
                if(Random::get<double>(0,1) < lambda)
                {
                    ...
                    poblacion_temporal[i].solucion[j] = poblacion[indice_seleccionado].solucion[j];
                    if(numero_elementos_seleccionados(poblacion_temporal[i]) != m)
                    {
                        //int random = Random::get<int>(0, DIMENSION_PROBLEMA);
                        //while(poblacion_temporal[i].solucion[random])
                        //{
                        //    random = Random::get<int>(0, DIMENSION_PROBLEMA);
                        //}
                        //poblacion_temporal[i].solucion[random] = true;
                       //Reparacion(n, m, s, matriz_datos);

                    }

                }else{
                    poblacion_temporal[i].solucion[j] = poblacion[i].solucion[j];
                }
            }
